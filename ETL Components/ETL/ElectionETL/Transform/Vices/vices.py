...
jsonVicefile = open('vices.json', 'r')
...
for candidate in candidates:
    ...
    if vices.has_key(candidate[0]):
        temp = vices[candidate[0]]
        found = False
        for ent in temp:
            otherlast = re.search('([a-zA-Z\ .]*) ([A-Z][a-zA-Z]*[a-z])( II)?', ent[0]).group(2)
            if lastname == otherlast:
                print "match"
                inserts.append([candidate[6], ent[1]])
                break
...
# insert into Vice values(CID, V_name);


popfile = open('populateVices.sql'.format(str(time.time())), 'w')
popfile.write('use PCD_Database;\n')


for insert in inserts:
    popfile.write("insert into Vice values({0}, \'{1}\');\n".format(insert[0], insert[1]))
